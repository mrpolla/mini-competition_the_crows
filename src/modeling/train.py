...
## Hyperparameter tuning
  ...
  best_model.booster_.save_model('lgbm_model.txt')
##
...
def create_model_with_CV(df):
  ...
  param_grid = {
    'n_estimators': [50, 100, 150],  # Lower and higher values for boosting rounds
    'learning_rate': [0.01, 0.05, 0.1],  # A good range of learning rates
    'max_depth': [3, 5]  # Shallow to deeper trees
  }
  ...
  # Perform Grid Search
  grid_search = GridSearchCV(estimator=xgb_model, param_grid=param_grid, cv=3, scoring='accuracy', verbose=10, n_jobs=-1)
